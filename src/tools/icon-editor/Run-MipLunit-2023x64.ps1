...
.DESCRIPTION
  ...
  then executes LabVIEW unit tests via the run-unit-tests helper (also 2023 x64). Writes a
...
$label = "mip-2023-labtest-{0}" -f (Get-Date -Format 'yyyyMMddTHHmmss')
...
$summaryData = [ordered]@{
  schema = 'integration/mip-lunit-2023@v1'
  label = $label
  resultsRoot = $resultsAbs
  status = 'pending'
  checks = [ordered]@{
    toolkit = [ordered]@{ status = 'pending'; version = 2023; bitness = 64; path = $null; labviewExe = $null; reason = $null }
...
try {
  ...
  Write-Host "[6a] ERROR: Rogue LV detection failed: $($_.Exception.Message)" -ForegroundColor Red
  throw
}
...
Ensure-VendorTools
$toolkitInfo = Test-VIAnalyzerToolkit -Version 2023 -Bitness 64
if (-not $toolkitInfo.exists) {
  Write-Host "[6a] VI Analyzer Toolkit check failed: $($toolkitInfo.reason)" -ForegroundColor Yellow
  Write-Host "[6a] Install 'LabVIEW 2023 VI Analyzer' via NI Package Manager and rerun." -ForegroundColor Yellow
  ...
  Write-Host ("[6a] Detected VI Analyzer Toolkit at {0}" -f $toolkitInfo.toolkitPath) -ForegroundColor DarkGray
}

$gcliInfo = Test-GCliAvailable
if (-not $gcliInfo.available) {
  Write-Host "[6a] g-cli check failed: $($gcliInfo.reason)" -ForegroundColor Yellow
  Write-Host "[6a] Install g-cli 3.x and ensure it is on PATH before rerunning." -ForegroundColor Yellow
  ...
  Write-Host ("[6a] g-cli detected ({0})" -f $gcliInfo.version) -ForegroundColor DarkGray
}
...
if (-not $serverInfo.enabled) {
  Write-Host "[6a] WARNING: LabVIEW VI Server appears disabled (server.tcp.enabled <> 1)." -ForegroundColor Yellow
}

[Environment]::SetEnvironmentVariable('MIP_EXPECTED_LV_VER','2023','Process')
  ...
  if (Test-Path -LiteralPath $detectScript -PathType Leaf) {
    ...
    if ($AutoCloseWrongLV.IsPresent -and $preflight -and $preflight.rogue.labview.Count -gt 0) {
      ...
      foreach ($entry in $preflight.liveDetails.labview) {
        ...
        if ($exe -and $exe -match 'LabVIEW.*2023' -and -not ($exe -match '\(x86\)')) { continue }
...
Write-Host "[6a] Analyzer-gated MIP (2023/64) + LUnit orchestration" -ForegroundColor Cyan
...
# Step 1: Analyzer-gated MIP (2023 x64)
...
try {
  ...
  if (Test-Path -LiteralPath $analyzerRoot -PathType Container) {
    ...
    if ($latestAnalyzerDir) {
      ...
      Write-AnalyzerFindings -AnalyzerDir $latestAnalyzerDir.FullName -Prefix '[6a]'
    }
  }
  Save-IntegrationSummary -Status 'pending'
} catch {
  $message = $_.Exception.Message
  Write-Host "[6a] ERROR: MissingInProject suite failed: $message" -ForegroundColor Red
  ...
  try {
    $latestAnalyzerDir = Get-ChildItem -LiteralPath $lastAnalyzerRun -Directory -Filter 'vi-analyzer-*' |
      Sort-Object LastWriteTimeUtc | Select-Object -Last 1
    if ($latestAnalyzerDir) {
      Write-Host ("[6a] See analyzer logs under {0}" -f $latestAnalyzerDir.FullName) -ForegroundColor DarkGray
      ...
      Write-AnalyzerFindings -AnalyzerDir $latestAnalyzerDir.FullName -Prefix '[6a]'
      if (Test-Path -LiteralPath $cliLogPath -PathType Leaf) {
        Write-Host "[6a] Last 10 lines from analyzer CLI log:" -ForegroundColor DarkGray
        Get-Content -LiteralPath $cliLogPath -Tail 10 | ForEach-Object { Write-Host ('[6a]   ' + $_) }
      }
    }
    Write-AnalyzerDevModeWarning -AnalyzerDir $latestAnalyzerDir.FullName -Prefix '[6a]' | Out-Null
  } catch {}
  if ($message -match '-350053') {
    Write-Host "[6a] VI Analyzer returned error -350053 (missing or broken toolkit). Install/repair the VI Analyzer Toolkit for LabVIEW 2023 (64-bit) and rerun." -ForegroundColor Yellow
  } elseif ($message -match 'VI Analyzer report not found') {
    Write-Host "[6a] Analyzer did not emit a report (exit code 1). Review the log above for failed tests, fix the violations, or scope the .viancfg to known-good VIs before rerunning." -ForegroundColor Yellow
  }
  Save-IntegrationSummary -Status 'failed'
  exit 3
}
...
$unitArgs = @(
  ...
  '-SupportedBitness','64',
  '-ProjectPath', $projAbs,
  '-ReportLabel', ('iconeditor-lunit-2023-{0}' -f (Get-Date -Format 'yyyyMMddTHHmmss'))
)
...
if ($unitSummaryLine) {
  Write-Host ("[6a] LUnit summary: {0}" -f ($unitSummaryLine.Trim())) -ForegroundColor DarkGray
}
if ($unitReportPath) {
  Write-Host ("[6a] LUnit report: {0}" -f $unitReportPath) -ForegroundColor DarkGray
...
if ($unitExitCode -ne 0) {
  Write-Host "[6a] LUnit run failed. Review output above for details." -ForegroundColor Red
