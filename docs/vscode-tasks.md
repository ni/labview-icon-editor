...
Curated workspace tasks for the local workflows we actually reach for. The headless/defaulted and release/draft helpers were removed to keep the palette small; use the underlying scripts in `scripts` or `scripts/` when you need automation.

- **Analyze VI Package (Pester)** – runs the analyzer (`scripts/analyze-vi-package/run-local.ps1`) against a `.vip` artifact; prompts for the artifact path and minimum LabVIEW version.
- **Build/Package VIP** – choose artifact type via `buildMode` input:
  - `vip+lvlibp`: runs `scripts/build/Build.ps1` (apply VIPC, build lvlibp(s), package VIP).
...
- **Analyze VI Package (Pester)**  
  - Runs `scripts/analyze-vi-package/run-local.ps1` against the provided `.vip` path.  
...
- **Build lvlibp (LabVIEW)**  
  ...
  - Calls `scripts/build-lvlibp/Build_lvlibp.ps1` to create the packed library for the selected bitness.
