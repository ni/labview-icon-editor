...
- SD bundle (lean-first): `scripts/source-distribution/Prepare-SDBundle.ps1` now ships the **lean preset as the canonical output**. After task 20 succeeds, run it with `-LeanExclude -OutputPath builds/artifacts/sd-bundle-lean.zip` (or let downstream tasks that call the script pick up the preset). The lean preset removes non-essential files (`.git`, `.github`, `.devcontainer`, `.secrets`, `.config`, `*.md`, `*.rst`, `*.adoc`, `*.txt`, `.editorconfig`, `.gitattributes`, `.gitignore`, `.pre-commit-config.yaml`, `.dockerignore`, `.yamllint`, `.lychee.toml`, `node_modules`) while keeping VS Code configs, tooling scripts/VIs, x-cli wiring, and the commit index. Omit `-LeanExclude` only when you intentionally need the full payload for forensic parity.
