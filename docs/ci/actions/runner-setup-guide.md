...
1. **Build VI Package**
    ...
    - Automatic publication is executed directly by `.github/workflows/ci.yml` for eligible `develop` pushes.
...
3. **Build VI Package**
    ...
    - Eligible merged-PR merge-commit pushes to `develop` publish automatically in the same `ci.yml` push run.
