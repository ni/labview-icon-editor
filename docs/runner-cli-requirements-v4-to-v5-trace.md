...
| V5-C7 | RC-PLAT-001, RC-PLAT-002, RC-CONF-001 | RC-PLAT-009, RC-PLAT-010, RC-CONF-010, RC-CONF-011 | Core | Clarifying | Define hosted cross-platform Core conformance evidence and explicit non-applicable handling for Windows-only command checks. | Execute Core checks on hosted Linux and hosted Windows and verify Windows-only checks are not-applicable on non-Windows platforms. |
