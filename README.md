...
- 🧪 **Runner CLI Workflow** – The `runner-cli` build, publish, smoke, and Docker validation steps are centralized in `.github/workflows/runner-cli.yml`.
...
2. **Issue Approval & Assignment** – Once the idea is approved, maintainers label the issue `Workflow: Open to contribution`. After you volunteer, a maintainer assigns the issue and sets up a branch such as `issue-123-short-description`. The workflow defined in [ci-composite.yml](.github/workflows/ci-composite.yml) runs on pushes/PRs to the supported branches.
...
4. **Implement & Test** – Develop your changes using LabVIEW. Test the editor manually in LabVIEW (in development mode) to ensure your changes work. Run any available unit tests. Make sure to apply the `runner_dependencies.vipc` file (located at `.github/actions/apply-vipc/runner_dependencies.vipc`) via VIPM or the scripts so that all required libraries (G-CLI, etc.) are present.
   For a fast pre-check of LabVIEW files without launching LabVIEW, run `pwsh -NoProfile -File .\Tooling\Run-ViValidate.ps1`. This uses `.lvversion` and `Tooling/pylavi/vi-validate.yml` (use `-ViValidateProfile legacy` for a broader legacy scan).
...
- **Tooling Reference:** Local script entrypoints (including `Run-ViValidate.ps1`) are documented in [Tooling/README.md](Tooling/README.md).
