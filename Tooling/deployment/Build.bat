...
call g-cli --lv-ver %LVVersion% --arch %SupportedBitness% -v "%RelativePath%\Tooling\deployment\Create_LV_INI_Token.vi" -- LabVIEW Localhost.LibraryPaths "%RelativePath%"
IF %ERRORLEVEL% NEQ 0 goto end
...
call g-cli --lv-ver %VIPackageLabVIEWVersion% --arch %SupportedBitness% -v vipb -- -av -b "%RelativePath%\Tooling\deployment\NI Icon editor.vipb"
IF %ERRORLEVEL% NEQ 0 goto end
...
call g-cli --lv-ver %LVVersion% --arch %SupportedBitness% -v "%RelativePath%\Tooling\deployment\Destroy_LV_INI_Token.vi.vi" -- LabVIEW Localhost.LibraryPaths
