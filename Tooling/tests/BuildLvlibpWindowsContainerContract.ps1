...
Describe 'Build LVLIBP Windows Container workflow contract' {
    BeforeAll {
        $repoRoot = (Resolve-Path -Path (Join-Path $PSScriptRoot '..\..')).Path
        $workflowPath = Join-Path $repoRoot '.github/workflows/build-lvlibp-windows-container.yml'
    ...
    It 'validates minimum LabVIEW version for Windows Docker' {
        $script:content | Should -Match '2026'
        $script:content | Should -Match 'minYear\s*=\s*2026|minimum.*2026q1'
    }
