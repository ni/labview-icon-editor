...
def append_telemetry_entry(
    ...
    A timestamp is added if not already present. By default this is a
    timezone-aware UTC ISO string. Set env var ``TELEMETRY_USE_LOCAL_TIME=1``
    to record local system time with offset instead.
    ...
    if "timestamp" not in entry:
        use_local = os.getenv("TELEMETRY_USE_LOCAL_TIME", "") == "1"
        if use_local:
