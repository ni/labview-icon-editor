...
- `{{QA_CMD_POSIX}}` or `{{QA_CMD_WIN}}` runs static checks, builds, and tests. Pre-commit is optional for local use.
...
## Testing Guidelines
Target .NET {{DOTNET_VERSION}} with xUnit. Name tests `Method_Scenario_Expectation`. Keep CLI output stable; prefer explicit asserts. Maintain coverage floors ({{COV_LINE}} line / {{COV_BRANCH}} branch) and update compliance docs when thresholds change.
