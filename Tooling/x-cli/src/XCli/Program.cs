...
public static class Program
{
    ...
    public static async Task<int> Main(string[] args)
    {
        ...
        if (simulateMode && !requiresReal)
        {
            var planResultSim = SimulationPlan.ForCommand(subcommand);
            var simulatorSim = new Simulator();
            var simResult = await simulatorSim.Execute(subcommand, planResultSim);
            var simLogMessage = planResultSim.Plan.Fail ? planResultSim.Plan.Message : string.Empty;
            logger.Log(subcommand, parsed.PayloadArgs, simLogMessage, simResult, sw.ElapsedMilliseconds);
            var thresholdSim = Env.GetInt("XCLI_MAX_DURATION_MS", DefaultMaxDurationMs);
            if (sw.ElapsedMilliseconds > thresholdSim)
            {
                Console.Error.WriteLine($"[{HostTag}] warning: duration {sw.ElapsedMilliseconds}ms exceeded {thresholdSim}ms");
            }
            return simResult.ExitCode;
        }
        if (subcommand == "echo")
        {
    ...
    private static void PrintProvenance()
    {
        ...
        var envTier = Env.Get("PROVENANCE_TIER");
        var envCacheKey = Env.Get("PROVENANCE_CACHEKEY");
