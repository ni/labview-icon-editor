...
bash
Copy
dotnet build Tooling/dotnet/VipbJsonTool/VipbJsonTool.csproj -c Release
The output binary (for your OS/runtime) will be in Tooling/dotnet/VipbJsonTool/bin/Release/<framework>/. Alternatively, use dotnet publish -c Release -r linux-x64 --self-contained -p:PublishSingleFile=true to produce the self-contained Linux binary as done in the Docker build.
...
Rebuild and inspect: If a Docker build fails, run it from the repo root with `docker build -f Tooling/seed/Dockerfile . --no-cache` to ensure you’re not using stale layers. Look at the build output for any “file not found” messages to identify what’s missing.
