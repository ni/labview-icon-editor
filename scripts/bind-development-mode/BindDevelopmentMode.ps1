...
# Structured helper to remove LocalHost.LibraryPaths entries with error reporting
function Remove-LibraryPathsEntriesSafe {
    param(
        [string]$LvVersion,
        [string]$Arch
    )
    ...
    try {
        ...
        $result.Message = ("LocalHost.LibraryPaths cleanup failed for {0}-bit LabVIEW {1}: {2}" -f $Arch, $LvVersion, $_.Exception.Message)
...
function Remove-LibraryPathsEntries {
    ...
    $result = [ordered]@{ Success = $true; Message = '' }
    try {
        $lvIniPath = Resolve-LVIniPath -LvVersion $LvVersion -Arch $Arch
        $lines = Get-Content -LiteralPath $lvIniPath -ErrorAction Stop
        if ($lines -isnot [System.Array]) { $lines = @($lines) }
        $pattern = "LocalHost\.LibraryPaths\d*="
        $filtered = $lines | Where-Object { [CmdletBinding()]
param(
    [Parameter(Mandatory = $true)]
    [string]$RepositoryPath,
...
# Idempotency guard: start from NONE by clearing existing tokens for target bitness/version before bind/cleanup/unbind.
if (-not $SummaryOnly -and $Mode -in @('bind','unbind','cleanup')) {
    if ($DryRun) {
        Write-Information ("Dry run: would clear LocalHost.LibraryPaths entries for {0}-bit LabVIEW {1} before {2}." -f ($bitnessList -join '/'), $lvVersion, $Mode) -InformationAction Continue
    }
    else {
        $clearError = $null
        foreach ($arch in $bitnessList) {
            $clearResult = Remove-LibraryPathsEntriesSafe -LvVersion $lvVersion -Arch $arch
        ...
        if ($clearError) {
            $precheckError = New-Object psobject -Property @{ Exception = New-Object System.Exception $clearError }
        }
    }
}
...
if ($precheckError) {
    ...
    foreach ($arch in $bitnessList) {
        ...
        if ($Mode -eq 'cleanup') {
            ...
            $ok = Remove-LibraryPathsEntriesSafe -LvVersion $lvVersion -Arch $arch
            $res.post_path = ''
            if ($ok.Success) {
                $res.status = 'success'
...
exit 0
 -notmatch $pattern }
        ...
        Write-Warning ("Failed to remove LocalHost.LibraryPaths entries for {0}-bit LabVIEW {1}: {2}" -f $Arch, $LvVersion, [CmdletBinding()]
param(
    [Parameter(Mandatory = $true)]
    [string]$RepositoryPath,
...
if (-not $SummaryOnly -and $Mode -in @('bind','unbind','cleanup')) {
    if ($DryRun) {
        ...
        foreach ($arch in $bitnessList) {
            $cleared = Remove-LibraryPathsEntries -LvVersion $lvVersion -Arch $arch
            if (-not $cleared) {
                Write-Warning ("Idempotency clear failed for {0}-bit LabVIEW {1}; continuing with existing entries." -f $arch, $lvVersion)
            }
        }
    }
}
...
.Exception.Message)
        $result.Success = $false
        $result.Message = ("LocalHost.LibraryPaths cleanup failed for {0}-bit LabVIEW {1}: {2}" -f $Arch, $LvVersion, [CmdletBinding()]
param(
    [Parameter(Mandatory = $true)]
    [string]$RepositoryPath,
